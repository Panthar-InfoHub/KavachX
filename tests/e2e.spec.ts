...
test.describe('Kavach X - End-to-End Tests', () => {
  ...
  // ------------------------------------------------------------------
  // 3. KAIROS PAGE TESTS (NEW!)
  // ------------------------------------------------------------------
  test.describe('Kairos Page', () => {
    ...
    test('should load correctly and have proper SEO metadata', async ({ page }) => {
      await expect(page).toHaveTitle('Kairos | Smart CCTV Security Device by Kavach X | Kavach X');

      const schemaScript = page.locator('script[type="application/ld+json"]');
      await expect(schemaScript).toHaveCount(1);
    });

    test('should display the kairos hero section and CTA', async ({ page }) => {
      // Validating using getByRole
      const title = page.getByRole('heading', { level: 1, name: /Kairos/i });
      await expect(title).toBeVisible();
  ...
  test.describe('Navigation', () => {
    ...
    test('should navigate from Home to Kairos via Footer', async ({ page }) => {
      const edgeBoxLink = page.getByRole('link', { name: 'Kairos', exact: true }).first();
