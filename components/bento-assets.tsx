...
export function RiskScannerAsset() {
  const risks = [
    { label: "STRUCTURAL", level: 0.3, status: "low" },
    { label: "FIRE", level: 0.15, status: "low" },
    { label: "EVACUATION", level: 0.6, status: "med" },
    { label: "SEISMIC", level: 0.1, status: "low" },
  ];
  return (
    <div className="w-full max-w-sm relative">
      ...
      <div className="mt-4 flex items-center gap-2">
        <motion.div
          className="w-1.5 h-1.5 rounded-full bg-blue-400 shadow-[0_0_6px_#60a5fa]"
          animate={{ opacity: [1, 0.3, 1] }}
          transition={{ duration: 2, repeat: Infinity }}
        />
        <span className="text-[9px] text-white/25 font-mono tracking-widest">SCANNING · 847 SENSORS ACTIVE</span>
      </div>
    </div>
  );
}
